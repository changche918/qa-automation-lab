...
print(get_response)                                          # 印出 Response 物件，例如 <Response [200]>
print(f"狀態碼: {get_response.status_code}")                # 200 = 成功；4xx = 客戶端錯誤；5xx = 伺服器錯誤
print(f"內容類型: {get_response.headers.get('Content-Type')}") # 告訴你回應是 JSON、HTML 還是圖片等
print(f"回應內容:\n{get_response.text}")                    # 純文字格式的回應內容
...
# requests.post(post_url, json={"name": "Ryan"}) # ← 傳 JSON，Content-Type 自動設為 application/json
# requests.post(post_url, data={"name": "Ryan"}) # ← 傳表單，Content-Type 為 application/x-www-form-urlencoded
...
patch_response = requests.patch(
    patch_url,
    json={"email": "new@example.com"},  # 只改 email 欄位，其他欄位不動
    timeout=5
)
...
put_response = requests.put(
    put_url,
    json={"name": "Ryan", "email": "ryan@xxx.xxx.xxx"},  # 整筆資料都要給
    timeout=5
)
