...
1. [必做] 請 AI 計算跑 routing 時間區段是怎麼算的 ? (e.g. 冷門時段? 依據?)
    ...
    - * 補充 : 任何事情都想 3 秒再回答 !
    - * 補充 : 先保持著遲疑的態度，務必要查證跟佐證，避免一開始就相信對方的說法 ( 以免造成團隊性重大失誤 )
2. [必做] 把 claude AI 用  skill.md 檔內建立
    - 把 Instructions 內容寫進去，避免每次都撈取太多
3. [必做] log 檔優化
    - [必做] 太多檔案分資料夾 - 用年、日、月 (file_manager_daily.py)
    - [X] 寄 LINE 到手機
    - [X] 寄 E-mail
4. [必做] 自動授權怎麼寫 : 加到提示詞是一種解法
5. [必做] claude ai 嘗試 API + Web 跑 ( 看能不能在雲上跑，嘗試解決 )
6. [星期二事情做完再做] 裝 appium 或 jenkins